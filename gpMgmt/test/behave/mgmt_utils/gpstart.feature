...
#          And the cluster is returned to a good state
    ...
    @concourse_cluster
    @demo_cluster
    Scenario Outline: "gpstart -a" accepts all (non-super user and utility mode) connections
        Given the database is running
#          And the user runs psql with "-c 'create user foouser login;'" against database "postgres"
#          And the user runs command "echo 'local all foouser trust' >> $COORDINATOR_DATA_DIRECTORY/pg_hba.conf"
#          And the database is not running
...
#
#         When The user runs psql <args> in <DB> in utility mode
#         Then command should return a return code of <return_code>
#
...
#         Then command should return a return code of 0
         ...
         When The user runs psql <args> in <DB> in <utility_mode>
         Then command should return a return code of <return_code>
...
#         And "gpstop -ai" should return a return code of 0
        ...
        Examples:
         | test scenarios                            | args               | DB       | utility_mode   | return_code | DB login      | error message |
         | super user connections in Utility mode    | -c '/l'            | postgres | 1           | 0           | success       | None          |
         | non-super user connections in Utility mode| -u foouser -c '/l' | postgres | 1           | 0           | success       | None          |
         | super user connections                    | -c '\l'            | postgres | 0          | 0           | success       | None          |
         | non-super user connections                | -u foouser -c '/l' | postgres | 0          | 0            | success       | None          |
