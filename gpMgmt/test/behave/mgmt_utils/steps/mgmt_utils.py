...
@when('The user runs psql {query} in {dbname} in {utility_mode}')
@then('The user runs psql {query} in {dbname} in {utility_mode}')
def impl(context, query, dbname, utility_mode):
    psql_cmd = ""
    if utility_mode:
        psql_cmd = "export PGOPTIONS=\'-c gp_role=utility\'; "
    psql_cmd = psql_cmd + "psql -d \'%s\' %s;" % (dbname, query)
    print(psql_cmd)
